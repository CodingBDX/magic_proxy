...
with open(filePath3) as f:
    ...
    for line in f:
        num_lines += 1
os.chdir(".")
print("proxy's number:", num_lines)
...
# Borderify image
        ...
        if answer3 == 'no':
            pad = 0 # Pad image by 1/8th of inch on each edge
            bordertol = 0 # Overfill onto existing border by 16px to remove white corners
            im_padded = np.zeros([im.shape[0] + 2 * pad, im.shape[1] + 2 * pad, 3])
...
shutil.copyfile(path2 + '/' + 'A4_cards.pdf', filePath1 + '/' + answer1 + '.pdf')
